...
public sealed class EnemyDataRepository
{
    ...
    // DataId로 적 정보 찾기
    public async UniTask<EnemyData> GetByDataIdAsync(string dataId, CancellationToken token)
    {
        ...
        // 데이터 ID에 맞는 적 정보 취득
        enemyDataById.TryGetValue(dataId, out EnemyData enemyData);
        return enemyData;
    }
            ...
            foreach (EnemyData enemyData in loadedEnemyData)
            {
                ...
                if (enemyDataById.ContainsKey(enemyData.DataId))
                {
                    throw new InvalidOperationException(
                        $"Duplicate enemy DataId: {enemyData.DataId}");
                }
